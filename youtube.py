...
st.set_page_config(page_title="Youtube Data Harvesting and Warehousing",
                   ...
                   menu_items={'About': """# This app is created by CHETHAN B RAJ. This is an *extremely* cool apP!"""})
...
client = pymongo.MongoClient("mongodb+srv://chethann:Chethu1999@newyoutubedata.8cnxktn.mongodb.net/"
                             "?retryWrites=true&w=majority")
db = client.new_youtube_data
# CONNECTING WITH MYSQL DATABASE
mydb = psycopg2.connect(
    host="localhost",
    port=5432,
    database="newyoutubee",
    user="postgres",
...
api_key = "AIzaSyAQmFxhZnZtXnn_Q996rsCMQhd_IeKjAf8"  # "AIzaSyAQmFxhZnZtXnn_Q996rsCMQhd_IeKjAf8"
...
if selected == "Home":
    # Title Image
    image = Image.open(r'C:\Users\CHETHU\OneDrive\Desktop\330685-P9SESK-66.jpg')
    st.image(image, caption='YOUTUBE DATA')
    ...
    col1.markdown(
        "## :orange[Overview] : Retrieving the Youtube channels data from the Google API, storing it in a MongoDB as data lake, migrating and transforming data into a SQL database,then querying the data and displaying it in the Streamlit app.")
    col2.markdown("#   ")
    col2.markdown("#   ")
    col2.markdown("#   ")
...
if selected == "Extract & Transform":
    tab1, tab2 = st.tabs(["$\huge 📩 EXTRACT $", "$\huge🛰️ TRANSFORM $"])
...
if selected == "View":
    ...
    elif questions == ('6. What is the total number of likes and dislikes for each video, '
                       'and what are their corresponding video names?'):
    ...
    elif questions == ('9. What is the average duration of all videos in each channel, '
                       'and what are their corresponding channel names?'):
        mycursor.execute("""SELECT channel_name AS Channel_Name,
                            duration AS "Average_Video_Duration (mins)"
                            FROM videos""")
    ...
    elif questions == ('10. Which videos have the highest number of comments, '
                       'and what are their corresponding channel names?'):
